                ...
                if step_type == "navigate":
                    ...
                    if not grounding.found:
                        if replans_used >= self.max_replans:
                            raise AgentError(
                                f"Step {i} ({step_type} -> {target_desc!r}) not found on page and "
                                f"replan budget ({self.max_replans}) exhausted."
                            )
                        replans_used += 1
                        await self.emit("replanning", index=i, step=step, reason="target not found")
